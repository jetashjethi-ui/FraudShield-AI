...
FraudShield AI is a fraud detection system built on the IEEE-CIS Fraud Detection dataset (590,540 transactions). It processes raw transaction data through an 11-step pipeline that includes feature engineering, model training, explainability analysis, and automated reporting.

The system uses two complementary approaches:
- **Supervised learning**: 6 models trained on labeled fraud/non-fraud data, combined through a stacking ensemble
- **Unsupervised learning**: A deep autoencoder that learns normal transaction patterns and flags anomalies
...
Step 2: Feature Engineering
    └─ 17 detection layers + graph features + target encoding
Step 3: Data Preparation
    └─ Train/test split (80/20, stratified) + SMOTE oversampling
Step 3.5: Optuna Tuning
    └─ 30-trial Bayesian optimization for XGBoost + LightGBM
Step 4: Model Training
    └─ 5-fold OOF stacking with 6 base models + meta-learner
...
### What it does
A 9-page interactive Streamlit dashboard for exploring all results.

### Pages

1. **Dashboard** — Overview with key metrics (AUC, F1, total transactions), risk tier distribution, and summary charts
2. **Live Detector** — Input transaction details manually and get a real-time risk score with explanations
3. **Models** — Compare all models side-by-side with ROC curves and confusion matrices
4. **Analytics** — Fraud patterns by hour, product category, and amount distribution
5. **Flagged** — Table of the highest-risk flagged transactions with explanations
6. **ROI Calculator** — Interactive sliders to compute cost-benefit analysis for different bank sizes and fraud rates
7. **Robustness** — Adversarial test results with pass/fail indicators
8. **Simulator** — Generate random transactions with configurable fraud probability, watch them get scored in real-time with risk bars and factor explanations
9. **Autoencoder** — Architecture visualization, reconstruction error comparison (normal vs fraud), and explanation of how the autoencoder works
